...
class terra(object):
    ...
    @classmethod
    def pacha(self, feh, mass, data_input=None, model=None, Mcon=None, chondrites=None, data_output=None):
        '''
        Args
        ...
        seab  = pd.read_csv('../data/seab_data.csv')
        
        #chondrites
        #the user can choose the chondrites type: CI, CM, CO, CV, H, L, LL, EH, EL
        met   = pd.read_csv('../data/Chondrites.csv', usecols=[chondrites])
        ...
        for i in trange(len(merged_tab['Mass_CM']), desc='searching the best solution'):
            eacm1 = []
            TME   = [] # Nmet + Nearth
